...
counts_matrix_file_name <- "counts_matrix.tsv"
...
- create clusters 
```{r}
n_clusters <- 30 # 30 is optimal for a cohort size of ~20,000 samples
...
```{r}
choice <- 1
sorted_clusters <- cluster_results$cluster_labels[[choice]]
...
write.table(membership$membership_file, "/path/to/write/membership_file/", sep="\t", col.names = TRUE, row.names = FALSE, quote=FALSE)
```
...
2/1/2021 - develop tools to cluster new samples to old clusters
```{r}
ssm <- read.table("C:/Users/iwong/Documents/MGH/ukbb/09_remainders/sample_set_membership.tsv", sep="\t", header=TRUE)
old_membership <- read.table("C:/Users/iwong/Documents/MGH/ukbb/09_remainders/ukbb_cluster_memb.tsv", sep="\t", header=TRUE, stringsAsFactors = FALSE)
pca <- read.table("C:/Users/iwong/Documents/MGH/ukbb/09_remainders/ukbb.200k.pca.1.tsv", sep="\t", header=TRUE)

old_membership$cluster <- old_membership$membership.sample_set_id %>% str_extract_all("(A|B)_\\d*") %>% unlist

sampleToCluster <- HashMap$new()
sampleToCluster$populate(old_membership, 2, 3)
clusterToPCAIndexes <- HashMap$new()
for(i in seq_down(pca)){
  currentSample <- row.names(pca)[i]
  
  if(sampleToCluster$containsKey(currentSample)){
    clusterToPCAIndexes$append(sampleToCluster$get(currentSample), i)  
  }
}

cluster_centers <- do.call(rbind, lapply(old_membership$cluster %>% unique, function(x){
  message(x)
    colMeans(pca[clusterToPCAIndexes$get(x), ])  
}) ) %>% as.data.frame()
row.names(cluster_centers) <- old_membership$cluster %>% unique

bool_samples_to_merge <- !(row.names(pca) %in% old_membership$sample)
samplesToMerge <- row.names(pca)[bool_samples_to_merge]
sampleToPCAIndex <- HashMap$new()
for(i in seq_down(pca)){
  sampleToPCAIndex$put(row.names(pca)[i], i)
}
nearest_clusters <- lapply(samplesToMerge, function(x){
  dists <- rbind(pca[sampleToPCAIndex$get(x) ,], cluster_centers) %>% dist() %>% as.matrix() 
  top_row <- dists[1, -1] %>% sort %>% names
  closest_cluster <- top_row[1]
  return(closest_cluster)
})  
names(nearest_clusters) <- samplesToMerge
nearest_clusters %>% unlist %>% table

memb <- data.frame(`membership:sample_set_id`=paste0("cluster_additional_" , unlist(nearest_clusters), "_CASE" ), sample=samplesToMerge, check.names = FALSE, stringsAsFactors = FALSE)
write.table(memb, "C:/Users/iwong/Documents/MGH/ukbb/09_remainders/memb_ukbb_new.tsv", sep="\t", col.names = TRUE, row.names = FALSE, quote=FALSE)



files <- list.files("C:/Users/iwong/Documents/MGH/ukbb/09_remainders/abtch_new_01/batch_new_01/", pattern=".exons.counts.tsv", full.names = TRUE, recursive = TRUE)
...
merged_counts$sex <- ifelse(merged_counts$y >= 5, "Y", "X")

ggplot(merged_counts, aes(x=x, y=y, color=sex)) + 
  geom_point() + 
  xlab("normalized chrX counts") + 
  ylab("normalized chrY counts") + 
  ggtitle("normalized chrX and chrY counts, epi")

merged_counts$additional_sex_cluster <- lapply(seq_down(merged_counts), function(x) {
  add_clust <- memb$`membership:sample_set_id`[memb$sample == merged_counts$sample[x]]
  return(add_clust %>% str_replace_all("CASE", paste0(merged_counts$sex[x], "_CASE")) %>% unlist)
}) %>% unlist

memb_sex <- data.frame(`membership:sample_set_id`= merged_counts$additional_sex_cluster, sample=merged_counts$sample, check.names = FALSE, stringsAsFactors = FALSE)
write.table(memb_sex, "C:/Users/iwong/Documents/MGH/ukbb/09_remainders/memb_ukbb_new_sex.tsv", sep="\t", col.names = TRUE, row.names = FALSE, quote=FALSE)
...
wd <- "C:/Users/iwong/Documents/MGH/ukbb/09_remainders/sample_set_counts_all/"
...
samples <- exons %>% basename %>% str_replace_all(".exons.counts.tsv", "") %>% unlist
...
df_new <- data.frame(`entity:sample_id` = samples, 
                     exons = exons,
                     check.names=FALSE)

write.table(df_new, paste0(wd, "sample_with_exons.tsv"), sep="\t", col.names = TRUE, row.names = FALSE, quote=FALSE)

sse <- read.table("C:/Users/iwong/Documents/MGH/ukbb/09_remainders/sample_set_file_configs/sse.tsv", sep="\t", header=TRUE, check.names = FALSE, fill=TRUE)
sse$counts_exons_all <- NULL
sse$`entity:sample_set_id` <- sse$`entity:sample_set_id` %>% str_replace_all("cluster", "cluster_additional") %>% unlist %>% str_replace_all("super_", "")

bool1 <- sse$`entity:sample_set_id` %in% memb$`membership:sample_set_id`
bool2 <- sse$`entity:sample_set_id` %in% memb_sex$`membership:sample_set_id`

sse_new_new <- sse[bool1 | bool2, ]
write.table(sse_new_new, "C:/Users/iwong/Documents/MGH/ukbb/09_remainders/sample_set_file_configs/sse_new_new.tsv", sep="\t", col.names = TRUE, row.names = FALSE, quote=FALSE)
