...
clusterPrimaryClusters <- function(pca_loads, n_clusters=30, my_seed=123, wd ="", USE_METRICS=FALSE, LOW_MEMORY_MODE=FALSE, LOAD_DIST=FALSE) {
  set.seed(my_seed)
  ...
  if(LOAD_DIST == FALSE){
    ...
    print("eucl!")
    if(LOW_MEMORY_MODE){
      save(dist_eucl, file=paste0(wd, "/dist_eucl.rda"))
      rm(dist_eucl)
    }
    ...
    print("maxi!")
    if(LOW_MEMORY_MODE){
      save(dist_maxi, file=paste0(wd, "/dist_maxi.rda"))
      rm(dist_maxi)
    }
    ...
    print("canb!")
    if(LOW_MEMORY_MODE){
      save(dist_canb, file=paste0(wd, "/dist_canb.rda"))
      rm(dist_canb)
    }
  }
  ...
  if(LOW_MEMORY_MODE){
    cuts <- lapply(seq_down(results), function(i) {
      print(i)
      if(results$distance[i] %>% as.character() == "euclidean"){
        load(paste0(wd, "/dist_eucl.rda"))
      } else if(results$distance[i] %>% as.character() == "maximum"){
        load(paste0(wd, "/dist_maxi.rda"))
      } else {
        load(paste0(wd, "/dist_canb.rda"))
      }
      cut_avg <- cutree(hclust(switch(results$distance[i] %>% as.character(), "euclidean"=dist_eucl, "maximum"=dist_maxi, "canberra"=dist_canb), method=results$agglomeration[i]), k=n_clusters)
      temp_names_srt <- names(srt(cut_avg))
      suppressWarnings(rm(dist_eucl, dist_maxi, dist_canb))
      gc()
...
relabelSmallClusters <- function(pca, n_clusters, sorted_clusters, seed = 123, threshold = 200){
  ...
  cluster_centers <- do.call(rbind, lapply(1:n_clusters, function(i){
    message(i)
    colMeans(pca[sorted_clusters==i, ])
  }) ) %>% as.data.frame()

  tab <- table(sorted_clusters)
  which_clusters_to_relabel <- which(tab < threshold) %>% unname
  clusters_to_relabel <- pca[sorted_clusters %in% which_clusters_to_relabel, ]
...
clusterOnSex <- function(wd, membership_file, seed = 123, merged_counts = FALSE){
  set.seed(seed)
  if(merged_counts[1] == FALSE){
    files <- list.files(wd, pattern=".exons.counts.tsv", full.names = TRUE, recursive = TRUE)

    # cluster <- makeCluster(detectCores()-1)
    # clusterExport(cl=cluster, varlist=c("files", "str_replace_all", "fread"), envir = environment())
    ...
    cluster <- makeCluster(detectCores()-1)
    clusterExport(cl=cluster, varlist=c("files", "str_replace_all", "fread"), envir = environment())
    merged_counts <- parLapply(cluster, files, function(x){
      dft <- fread(cmd = paste("grep -P \"(^chrX)|(^chrY)|(^X)|(^Y)\"", x), col.names = c("CONTIG", "START", "END", "COUNT"), header = FALSE)
      dft <- dft[!(dft$START<=2781479 & dft$END>=10001),] # remove PAR1
      dft <- dft[!(( (dft$CONTIG=="chrX" | dft$CONTIG=="X") & dft$START<=156030895 & dft$END>=155701383) |
                     ((dft$CONTIG=="chrY" | dft$CONTIG=="Y") & dft$START<=57217415 & dft$END>=56887903)),] # remove PAR2
      data.frame(xcount = sum(dft$COUNT[dft$CONTIG=="chrX" | dft$CONTIG=="X"]),
                 ycount = sum(dft$COUNT[dft$CONTIG=="chrY" | dft$CONTIG=="Y"]),
                 sample = x %>% basename %>% str_replace_all(".exons.counts.tsv", ""), stringsAsFactors = FALSE)
    }) %>% do.call(rbind, .)
    stopCluster(cluster)
