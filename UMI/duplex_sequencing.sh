...
java -Xmx${MEM} ${FGBIO_PATH} ExtractUmisFromBam --input=${STEP_02_OUTPUT} --output=${STEP_03_OUTPUT} --read-structure=${UMI1} ${UMI2} --molecular-index-tags=ZA ZB --single-tag=RX
...
STEP_06_OUTPUT_02=${TMP_DIR}${NAME}_step
06_R2.fastq
...
java -Xmx${MEM} ${FGBIO_PATH} GroupReadsByUmi --strategy=paired --input=${STEP_08_OUTPUT} --output=${STEP_09_OUTPUT} --raw-tag=RX --assign-tag=MI --min-map-q=10 --edits=1
...
java -Xmx${MEM} ${FGBIO_PATH} CallDuplexConsensusReads --input=${STEP_09_OUTPUT} --output=${STEP_10_OUTPUT} --error-rate-pre-umi=45 --error-rate-post-umi=30 --min-input-base-quality=10 --threads=${NT} --min-reads=0
bsub "${BSUB_ARGS[@]}" java -Djava.io.tmpdir=${TMP} -Xmx${MEM} -jar ${PICARD_PATH} EstimateLibraryComplexity -I ${STEP_10_OUTPUT} -O ${STEP_10_METRICS_01}
